<script lang="ts">
	...
	async function handleSubmit() {
		...
		try {
			...
			const payload: { year: number; theme: string; leaders: Record<string, string | string[]> } = {
				year,
				theme,
				leaders: {}
			};
	...
	<div class="form-card">
		...
		<form on:submit|preventDefault={handleSubmit}>
			...
			<div class="form-section">
				...
				<div class="form-row">
					{#each ['chairman', 'deputyChairman', 'treasurer', 'secretary'] as field (field)}
						<div class="form-group">
							<label for={field}>{formatLabel(field)}</label>
							<input
								type="text"
								id={field}
								bind:value={singleFields[field]}
			...
			<div class="form-section">
				...
				<div class="form-row">
					{#each ['prayerCoordinator', 'choirMaster', 'assistantChoirMaster', 'talentsCoordinator', 'assistantTalentsCoordinator'] as field (field)}
						<div class="form-group">
							<label for={field}>{formatLabel(field)}</label>
							<input
								type="text"
			...
			<div class="form-section">
				...
				<div class="form-row">
					{#each Object.keys(arrayFields) as fieldKey (fieldKey)}
						<div class="form-group">
							<label for={fieldKey}>{formatLabel(fieldKey)}</label>
							<div class="dynamic-list" id={fieldKey}>
								{#each arrayFields[fieldKey] , index (index)}
									<div class="dynamic-item">
										<input
											type="text"
											bind:value={arrayFields[fieldKey][index]}
											placeholder="Full Name"
										/>
										<button
											type="button"
											class="btn-remove"
											on:click={() => removeArrayItem(fieldKey, index)}>×</button
										>
									</div>
								{/each}
								<button type="button" class="btn-add" on:click={() => addArrayItem(fieldKey)}
									>+ Add Official</button
								>
							</div>
						</div>
